...
a=list(map(int, input().split())) # 입력받은 데이터를 int 타입의 리스트로 변형
res = set() #set은 중복을 제거하는 자료구조
for i in range(n): ## 카드를 뽑는 3중 for 문 i에서 처음 카드 j 그다음 카드 m 마지막카드 뽑기 - 중복을 방지하면서 뽑음
    for j in range(i+1,n):
        for m in range(j+1,n):
            res.add(a[i]+a[j]+a[m])
res = list(res) # sort를 위해 list형변수로 바꿈
res.sort(reverse=True) #내림차순으로 바꿈
print(res)
print(res[k-1])
