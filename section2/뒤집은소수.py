...
def reverse(x):
    res=0
    while x>0:
        t=x%10 # 나머지를 찾아서 t에 저장
        res=res*10 + t#기존에있던것 10 곱해주고 t를 더함
        x=x//10 #다음 반복을 위해 몫으로 나눔 이러한것들이 반복되어 거꾸로도딘 정수가 나옴
    return res

def isPrime(x):
    if x ==1:
        return False
    for i in range(2,x//2+1): #절반까지만 확인해도 돼서 범위 이런식으로 해놓음
        if x%i==0:
            return False
    else:
        return True
